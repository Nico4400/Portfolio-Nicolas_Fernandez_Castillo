  ...
  //     brief: "Control de planillas automatizadas para detectar errores en el manejo de los usuarios. Optimizar la comunicación dentro de la dirección y con el resto de las mismas.",
  //     descriptionPoints: [
  ...
  //     brief: "Lideré la optimización de procesos y la implementación del módulo financiero en SAP ERP, impactando la eficiencia de la certificación de obra pública.",
  //     descriptionPoints: [
  //         "Responsable de la operación segura y eficiente de los elevadores de sillas de montaña y sus usuarios. Como también del mantenimiento de todos los aspectos operativos de la elevación asignada.",
  //     ]
  //   }, 
  //   {
  //     id: 5,
  ...
  //         "Estudio de métodos y tiempos para la optimización de procesos productivos.",
  //         "Análisis económico-financiero y evaluación de proyectos de inversión.",
  ...
  //         "Desarrollo de habilidades en estadística, investigación operativa y modelado de sistemas."
  //     ]
  //   },
  //   {
  //     id: 6,
  ...
  //     id: 8,
  //     type: "Emprendimiento",
  //     organization: "Nutrimas",
  //     logo: "/logo_nutrimas.png", // Si no tienes un logo, lo dejaremos en blanco o nulo
...
const Experience = () => {
    ...
    return (
        <section className={styles.experienceSection}>
            <h2 className={styles.sectionTitle}>Mi Trayectoria</h2>
            <p className={styles.sectionSubtitle}>
                Un recorrido por mi experiencia profesional, académica y emprendedora.
            </p>

            <div className={styles.timelineContainer}>
                {experienceData.map((experience, index) => (
                    <div key={experience.id} className={`${styles.timelineItem} ${index % 2 === 0 ? styles.left : styles.right}`}>
                        ...
                        <div className={styles.timelineCard}>
                            <div className={styles.cardHeader}>
                                <h3 className={styles.cardTitle}>{experience.position}</h3>
                                <p className={styles.cardSubtitle}>{experience.organization} | {experience.period}</p>
                                <p className={styles.cardBrief}>{experience.brief}</p>
                                {experience.descriptionPoints && experience.descriptionPoints.length > 0 && (
                                    <button
                                        className={styles.expandButton}
                                        onClick={() => toggleExpand(experience.id)}
                                        aria-expanded={expandedCardId === experience.id}
                                        aria-controls={`details-${experience.id}`}
                                        aria-label={expandedCardId === experience.id ? "Contraer detalles" : "Expandir detalles"}
                                    >
                                        {expandedCardId === experience.id ? <FaMinus /> : <FaPlus />}
                                        <span className={styles.expandButtonText}>
                                            {expandedCardId === experience.id ? 'Ver menos' : 'Ver más'}
                                        </span>
                                    </button>
                                )}
                            </div>
                            ...
                            <div
                                ...
                                className={`${styles.cardDetails} ${expandedCardId === experience.id ? styles.expanded : ''}`}
                            >
                                <ul>
